...
class UEAloader(Dataset):
    def __init__(self, args, data_zip, flag=None):
        ...
        self.data = np.transpose(self.data, (0, 2, 1))  # final (n_samples, seq_len, channels)
    ...
    def __getitem__(self, ind):
        ...
        if self.flag.lower() == "train" and self.args.augmentation_ratio > 0:
            ...
            batch_x_np, label, _ = run_augmentation_single(batch_x_np, np.array([label]), self.args)
            batch_x = batch_x_np[0]

        return self.instance_norm(torch.from_numpy(batch_x).float()), torch.tensor(label).long()
