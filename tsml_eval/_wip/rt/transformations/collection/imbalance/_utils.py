...
import socket
from tsml_eval._wip.rt.transformations.collection.imbalance.metrics import *


def evaluate_data(ori_data, gen_data, dataset_name='dataset', model_name='model'):
    if ori_data.shape[1] == 1 and ori_data.ndim == 3:
        ori_data = np.transpose(ori_data, (0, 2, 1))
    if gen_data.shape[1] == 1 and gen_data.ndim == 3:
        gen_data = np.transpose(gen_data, (0, 2, 1))
    # Parse configs
    method_list = '[MDD,ACD,SD,KD,ED,DTW,t-SNE,Distribution]'
    ...
    if is_mac:
        ...
        roots = '/Users/qiuchuanhang/PycharmProjects/tsml-eval/local'
    else:
        print("[ENV] Unknown environment, go to iridis")
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        roots = '/scratch/cq2u24/tsml-eval'

    result_path = roots+'/result/'
    ...
    # Replace the block around line ~80 with this:
    if isinstance(result, dict):
        # result_path previously is roots + '/result/' — keep it as the directory
        results_dir = result_path
        os.makedirs(results_dir, exist_ok=True)

        result_file = os.path.join(results_dir, f'{combined_name}.json')

        def _json_default(o):
            # simple serializer for numpy / non-serializable objects
