...
#         self.n_neighbors = suggested_n_neighbors
#         unique, counts = np.unique(y, return_counts=True)
...
def simple_frequency_bin_smote(X, y, n_neighbors=3, top_k=3, freq_match_delta=1,
                               bandwidth=1, random_state=None, normalize_energy=False, n_samples_gen=None):
    """
    一个简化的频率域SMOTE过采样方法。

    该方法基于样本的主导频率相似性进行邻居选择，并在频率域进行插值以生成新的合成样本。
    仅支持单变量（univariate）时间序列。

    参数
    ----------
    X : np.ndarray
        输入时间序列数据，形状为 (n_samples, n_channels, n_timesteps) 或 (n_samples, n_timesteps)。
    y : np.ndarray
        输入标签，形状为 (n_samples,)。
    ...
    top_k : int, default=3
        每个样本考虑的前k个主导频率数。
    freq_match_delta : int, default=1
        允许两个样本被视为频率邻居的最大频率索引差。
    bandwidth : int, default=1
        频率域插值时的带宽。
    random_state : int, RandomState instance or None, default=None
        随机数生成器种子控制。
    normalize_energy : bool, default=False
        是否在频率域插值后将合成样本的能量归一化以匹配原始样本。
    ...
    # 将输入数据转换为 (n_samples, n_timesteps) 的形状
    if X.ndim == 3:
        X = X[:, 0, :]
    ...
    # 预先计算所有样本的前k个主导频率
    freq_all_samples = _compute_topk_frequencies(X, top_k)

    X_class = X
    y_type = y[0]
    freq_class = freq_all_samples
    X_gen = np.zeros((n_samples_gen, seq_len), dtype=X.dtype)
    y_gen = np.full(n_samples_gen, fill_value=y_type, dtype=y.dtype)

    for i in range(n_samples_gen):
        # 随机选择一个少数类样本作为基准
        idx = _random_state.randint(0, len(X_class))
        x_curr = X_class[idx]
        ...
        scores = np.zeros(len(X_class), dtype=int)
        for f in freq_curr:
            score = np.any(np.abs(freq_class - f) <= freq_match_delta, axis=1).astype(int)
            scores += score

        # 选择得分最高的n_neighbors个邻居（排除自身）
        topk_neighbors = np.argsort(-scores)[:n_neighbors + 1]
        topk_neighbors = topk_neighbors[topk_neighbors != idx]

        # 如果没有足够的邻居，发出警告并跳过
        if len(topk_neighbors) == 0:
            warnings.warn(f"找不到足够的频率相似邻居，跳过生成样本 {i}")
            continue

        # 随机选择一个邻居
        nn_idx = _random_state.choice(topk_neighbors)
        x_nn = X_class[nn_idx]
        ...
        # 遍历基准样本的主导频率，进行插值
        for f in freq_curr:
            # 限制插值权重alpha在[0.1, 0.9]之间
            alpha = _random_state.uniform(0.1, 0.9)

            # 在带宽范围内进行插值
            for idx_bin in range(f - bandwidth, f + bandwidth + 1):
                if 0 <= idx_bin < F_curr.shape[0]:
                    decay = 1.0 / (1.0 + abs(idx_bin - f))
                    F_synthetic[idx_bin] += decay * alpha * (F_nn[idx_bin] - F_curr[idx_bin])

        # 如果需要，归一化能量
        if normalize_energy:
            energy_curr = np.sum(np.abs(F_curr) ** 2)
            energy_synth = np.sum(np.abs(F_synthetic) ** 2)
            if energy_synth > 1e-8:
                scaling = np.sqrt(energy_curr / energy_synth)
                F_synthetic *= scaling
    ...
    return X_gen.reshape(-1, 1, seq_len), y_gen


def _compute_topk_frequencies(X, top_k):
    """为所有样本计算前k个频率索引。"""
    freq_features = []
    for x in X:
        fft_vals = np.fft.rfft(x)
        mag = np.abs(fft_vals)
        mag[0] = 0  # 忽略直流分量
        topk_idx = np.argpartition(mag, -top_k)[-top_k:]
        freq_features.append(topk_idx)
    return np.array(freq_features)
...
if __name__ == '__main__':
    ...
    print(f"原始数据集分布: {np.unique(y, return_counts=True)}")
    X_minority = X[y == 1]
    print(f"少数类样本数: {len(X_minority)}")
    y_minority = y[y == 1]
    X_resampled, y_resampled = simple_frequency_bin_smote(
        X_minority, y_minority, n_neighbors=3, top_k=10, freq_match_delta=2, normalize_energy=True, random_state=42,
        n_samples_gen=10
    )

    print(f"过采样后数据集形状: X={X_resampled.shape}, y={y_resampled.shape}")
    print(f"过采样后类别分布: {np.unique(y_resampled, return_counts=True)}")
