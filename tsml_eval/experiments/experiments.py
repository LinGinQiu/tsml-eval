...
def run_classification_experiment(
    ...
    import pickle
    def save_dict(data: dict, folder="results", prefix="exp"):
        """Save a dictionary to a file with timestamp."""
    ...
    folder = f'/scrathc/cq2u24/Results/'

    from tsml_eval.experiments import (
        get_data_transform_by_name,)
    for data_transform_name in ['ohit','esmote','adasyn','tsmote','smote']:
        ...
        if data_transforms is not None:
            ...
            for transform in data_transforms:
                ...
                transform_results = transform[0].fit_transform(X_train, y_train)
                fit_time = int(round(time.time() * 1000)) - start
                result_data = {'name': data_transform_name, 'fit_time': fit_time, 'dataset_name': dataset_name}
                file_path = save_dict(result_data, folder=folder, prefix=f"{data_transform_name}_{dataset_name}")
        ...
        file_path = save_dict(result_data, folder=folder, prefix=f"{classifier_name}_{dataset_name}")
        ...
        file_path = save_dict(result_data, folder=folder, prefix=f"{classifier_name}_{dataset_name}_resampled")
        from aeon.classification.convolution_based import MultiRocketHydraClassifier
        classifier = MultiRocketHydraClassifier()
        classifier_name = type(classifier).__name__
        start = int(round(time.time() * 1000))
